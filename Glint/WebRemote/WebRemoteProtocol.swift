...
private let webRemoteKeyLogger = Logger(subsystem: "app.glint", category: "WebRemoteKey")
...
/// Keychain-backed storage: encrypted at rest by the system, device-local
/// (never synced to iCloud), and readable after first unlock so the server can
/// restart unattended without prompting.
...
/// Process-lifetime storage. Drives the tests, and catches the case where the
/// Keychain refuses a write: losing persistence across a restart is an
/// acceptable degradation, writing the token back to disk in cleartext is not.
final class WebRemoteEphemeralSecretStorage: WebRemoteSecretStorage, @unchecked Sendable {
...
enum WebRemoteAccessKeyStore {
    static let account = "accessKey"

    /// Builds before the Keychain move kept the token here, in cleartext. It is
    /// only ever read now — to carry an existing install across — and erased in
    /// the same pass so the plaintext copy does not linger.
    private static let legacyDefaultsKey = "glint.webRemoteAccessKey"

    /// Holds the token for this process when the Keychain is unavailable, so a
    /// stop→start cycle doesn't silently invalidate every paired browser.
    private static let fallbackStorage = WebRemoteEphemeralSecretStorage()

    static func loadOrCreate(
        storage: WebRemoteSecretStorage = WebRemoteKeychainStorage.shared,
        legacyDefaults: UserDefaults = .standard
    ) -> String {
        if let existing = storage.secret(for: account), isValid(existing) {
            purgeLegacy(legacyDefaults)
            return existing
        }
        if let migrated = legacyDefaults.string(forKey: legacyDefaultsKey), isValid(migrated) {
            webRemoteKeyLogger.info("Migrating web remote access key out of plaintext defaults")
            let stored = store(migrated, in: storage)
            purgeLegacy(legacyDefaults)
            return stored
        }
        if let cached = fallbackStorage.secret(for: account), isValid(cached) {
            purgeLegacy(legacyDefaults)
            return cached
        }
        return reset(storage: storage, legacyDefaults: legacyDefaults)
    }
    ...
    static func reset(
        ...
        let stored = store(key, in: storage)
        purgeLegacy(legacyDefaults)
        return stored
    }

    private static func store(_ key: String, in storage: WebRemoteSecretStorage) -> String {
        if storage.setSecret(key, for: account) {
            fallbackStorage.removeSecret(for: account)
            return key
        }
        webRemoteKeyLogger.error("Keychain write refused; web remote access key will not survive a restart")
        fallbackStorage.setSecret(key, for: account)
        return key
    }

    private static func purgeLegacy(_ defaults: UserDefaults) {
